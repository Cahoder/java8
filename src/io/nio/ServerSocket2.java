...
public class ServerSocket2 {
        ...
        try {
            while(true) {
                ...
                while(selectionKeys.hasNext()) {
                    ...
                    // 这个已经处理的readyKey一定要移除。
                    // 如果不移除，就会一直存在在selector.selectedKeys集合中
    ...
    private static void readSocketChannel(SelectionKey readyKey) throws Exception {
        ...
        if(URLDecoder.decode(message.toString(), "UTF-8").contains("over")) {
            ...
            System.out.println("端口:" + resourcePort + "客户端发来的完整信息======completeMessage : " + URLDecoder.decode(completeMessage.toString(), "UTF-8"));
            ...
            System.out.println("端口:" + resourcePort + "客户端信息还未接受完，继续接受======message : " + URLDecoder.decode(message.toString(), "UTF-8"));
